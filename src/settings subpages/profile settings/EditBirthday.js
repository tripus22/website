...
function EditBirthday(props) {
    ...
    const CheckBlank = () => {
        return <e>Please enter a date!</e>
    }

    const UPDATE_USER_BIRTHDAY = gql `mutation update_user ($ID: Int!, $Date: date!) {
        update_User_by_pk(
    ...
    const SendData = () => {
        ...
        if (loading)
            return <p>Updating birthday...</p>

        if (data) {
            let userProfile = JSON.parse(sessionStorage.getItem('userProfile'));
            userProfile.User_DOB = birthdayInput
            sessionStorage.setItem('userProfile', JSON.stringify(userProfile));
            window.location.reload(false);
            return <p>Birthday updated!</p>
        }
        return null
            ...
            <Content theme={props.theme}>
                <SignUpForm>
                    ...
                    <Form onSubmit={handleKeypress}>
                        ...
                        <Row>
                            <SubmitButton onClick={handleClick}>Submit New Birthday </SubmitButton>
...
export default connect(mapStateToProps)(EditBirthday);
